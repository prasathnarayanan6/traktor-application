...
const AddMentor = async(req, res) => {
    ...
    if (!mentor_name || !mentor_description || !years_of_experience || !area_of_expertise || !current_designation || !institution || !qualification || !year_of_passing_out || !startup_associated || !contact_number || !email_address || !linkedIn_ID || !password) {
    ...
    {
        return res.status(410).send('Phone number is not valid');
    }
    else {
        try
        {
            const result = AddMentorModel(mentor_name, mentor_description, years_of_experience, area_of_expertise, current_designation, institution, qualification, year_of_passing_out, startup_associated, contact_number, email_address, linkedIn_ID, password)
        }
        catch(err)
        {
            console.log(err);
